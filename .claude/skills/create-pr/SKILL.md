...
3. **본문**: `.github/PULL_REQUEST_TEMPLATE.md` 형식을 그대로 채운다. 기준은 **이 PR 링크만 처음 본 사람(팀 외부·채용 리뷰 포함)도 배경 없이 읽고 이해**할 수 있는 자기완결적인 글이다. 단순 파일 나열 금지.
   ...
   - **Changes**: 주요 변경을 **왜 그렇게 했는지**(설계 결정·트레이드오프) 중심으로 설명한다. 필요하면 Before → After로 대비한다. 사소한 파일까지 나열하지 말고 핵심 위주로 묶는다
     - 파일이 여러 폴더에 걸쳐 새로 생긴 경우, 관심사 설명 아래에 **"구조 요약"** 블록(폴더·역할 트리)을 덧붙여 리뷰어가 배치를 빠르게 잡게 한다. 단, 이는 **드라이한 파일 나열이 아니라 폴더별 역할 요약 트리**여야 한다(파일별 상세는 CodeRabbit walkthrough가 커버). 예시:
       ```text
       app/reward/page.tsx            획득 화면 라우트
       src/components/features/reward/
         ├─ queries/                  API·스키마
         ├─ RewardUnlockFlow/         순차 오케스트레이션
         └─ hooks/useRewardUnlock     조회 후 이동 트리거
       ```
