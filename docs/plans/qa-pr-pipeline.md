...
> 1단계는 혼자 먼저 진행하므로 아래는 **임시 결정**이다. 팀원 검토 후 바뀔 수 있다.

- [x] **스펙 신뢰도 표시:** `status` 필드는 **보류**. 지금은 모든 스펙을 draft(현재 코드 동작 기준)로 간주한다
  - 지금 스펙은 "기획 확정본"이 아니라 "현재 코드 동작"이다. 이대로 QA하면 회귀는 잡지만 이미 있는 버그는 정상으로 검증한다
  - 검토된 문서가 생기면 그때 필드 도입을 다시 논의한다
- [x] **"버그 의심"·`[확인 필요]` 처리 정책:** 시나리오로 **검증하지 않고** "사람 확인 체크리스트"로만 출력한다
- [x] **QA 환경 제약 위치:** `.claude/skills/qa-pr/references/qa-policy.md`
- [x] **1단계 스킬 frontmatter:** `disable-model-invocation: true`, 인자는 PR 번호(없으면 현재 브랜치), 파일 쓰기 도구 없음
- [x] **작업 브랜치:** `chore/ai-pr-qa-agent-jyj` (`chore/ai-pr-qa-agent`에서 분기)
